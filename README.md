...
1. Use PLOTHIST mode to visualize the Wiberg bond orders from the previous step.
...
1. Use SELECT mode to separate molecules. This mode creates a directory with
   SMILES files corresponding to certain molecule [bins](#bins).
1. Use SELECT-ANALYZE mode to provide some statistics about the output of SELECT
   mode, as well as a bar graph of the counts of molecules in each bin.
1. Use SELECT-FINAL mode to select the smallest molecules from each bin created
   in SELECT mode.
...
and prop2.binary, and writes molecules of each bin to files in a directory
called my-output.
...
Selects the 2 smallest molecules from each SMILES file in the select-output
directory and writes them to the file select-final.smi.
...
redirect stderr to a file called status.txt when running dance.
